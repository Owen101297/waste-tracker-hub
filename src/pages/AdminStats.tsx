    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="container mx-auto p-4 md:p-6 space-y-6">
        <div className="flex items-center justify-between flex-wrap gap-3">
          ...
          <Button variant="outline" size="sm" onClick={handleExportExcel} disabled={exporting || data.length === 0}>
                  ...
                  <tbody>
                    {institutions.map((inst) => {
                      ...
                      const uniqueMonths = new Set(
                        instRecords.map((r) => `${r.year}-${r.month}`)
                      ).size;
                      const totalKg = instRecords.reduce(
                        (sum, r) =>
                          sum +
                          WASTE_COLUMNS.reduce(
                            (a, c) => a + Number(r[c.key] ?? 0),
                      ...
                      return (
                        <tr key={inst.id} className="border-b hover:bg-gray-50">
                          ...
                          <td className="p-2">{uniqueDays}</td>
                          <td className="p-2">{uniqueMonths}</td>
                          <td className="p-2">{totalKg.toFixed(2)} kg</td>
                          <td className="p-2">
                            <Badge
                              variant={
                                compliance === 'Alto'
